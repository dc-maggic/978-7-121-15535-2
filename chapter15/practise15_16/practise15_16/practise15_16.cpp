...
class Next_quote :public Disc_quote{
public:;
	   Next_quote() = default;
	   Next_quote(const std::string &book, double p, std::size_t qty, double disc) :Disc_quote(book, p, qty, disc) {}
	   double net_price(std::size_t n) const override;
};
double Next_quote::net_price(std::size_t n) const {
	if (n <= quantity)
		return n * (1 - discount)*price;
	else
